...
# Descripción: Vista para la opción multimedia.
...
# Listar
class Gen_MultimediaListView(ListView):
    model = Gen_Multimedia
    template_name = 'gen_multimedia/list.html'

    def get_queryset(self):
        emp_filter = self.request.session.get('AIGN_EMP_ID')
        if emp_filter:
            return Gen_Multimedia.objects.filter(mul_estado=1)
        return Gen_Multimedia.objects.all()

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = 'Reporte ' + self.model._meta.verbose_name
        context['url_form'] = reverse_lazy('gen:gen_multimedia_add') #add
        context['url_form_add'] = reverse_lazy('gen:gen_multimedia_add') #add
        context['url_form_edit'] = 'gen:gen_multimedia_edit' #edit
        ...
        for r in context['object_list']:
            r.lin_id = _e.encrypt(r.mul_id)
        return context


# Agregar
class Gen_MultimediaCreateView(CreateView):
    model = Gen_Multimedia
    form_class = Gen_MultimediaForm
    template_name = 'gen_multimedia/form.html'
    success_url = reverse_lazy('gen:gen_multimedia_list')

    # Asigna al kwargs la variable de session desde el formulario
    def get_form_kwargs(self):
        kwargs = super(Gen_MultimediaCreateView, self).get_form_kwargs()
        # #Roles
...
class Gen_MultimediaUpdateView(UpdateView):
    model = Gen_Multimedia
    form_class = Gen_MultimediaForm
    template_name = 'gen_multimedia/form.html'
    success_url = reverse_lazy('gen:gen_multimedia_list')
    ...
    # Asigna al kwargs la variable de session desde el formulario
    def get_form_kwargs(self):
        kwargs = super(Gen_MultimediaUpdateView, self).get_form_kwargs()
        # kwargs.update({'AIGN_OPCIONES': self.request.session['AIGN_OPCIONES']})
